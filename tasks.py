...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_ITENTIAL_xxx
namespace = Collection("nautobot_itential")
namespace.configure(
    {
        "nautobot_itential": {
            "nautobot_ver": "2.1.0",
            "project_name": "nautobot-itential",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_itential.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_itential.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_itential.nautobot_ver,
        "PYTHON_VER": context.nautobot_itential.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_itential.project_name}",
        f'--project-directory "{context.nautobot_itential.compose_dir}"',
    ]

    for compose_file in context.nautobot_itential.compose_files:
        compose_file_path = os.path.join(context.nautobot_itential.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_itential.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_itential.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_itential"
...
@task(
    help={
        "version": "Version of Nautobot Itential to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_itential'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_itential/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_itential.local):
